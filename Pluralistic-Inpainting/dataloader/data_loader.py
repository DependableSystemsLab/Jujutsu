...
class CreateDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        "interpret the coordinate from the path name and send to load_mask"
        ...
        try:
            ...
            print("setting default coordinate")
            coordinate = (112,112)

        mask = self.load_mask(img, index, coordinate, self.opt.mask_pert, self.opt.is_square_patch)
    ...
    def load_mask(self, img, index, coordinate, mask_percentage, is_square_patch):
        ...
        if mask_type == 4:
            return task.dot_mask(img=img, coordinate= coordinate, radius=51, percentage_of_pert=mask_percentage, is_square_patch=is_square_patch)
        # center mask
        if mask_type == 0:
