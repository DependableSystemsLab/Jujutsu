...
def main() -> None:
    ...
    model.fc = torch.nn.Linear(num_features, 307) # multi-class classification (num_of_class == 307)
    ...
    for i in range(input_size):
        ...
        if(args['extract_label']): 
            ...
            if( index[0] == int(clean_label) ):
                ...
                #print( files[i] )
            #else:
            #    print("\t\tpredictions on {}: {}, label: {} | pred on unpainted image {}".format(files[i].replace(args['img_folder'],''), index, clean_label, truth_index[0]) , flush=True)
